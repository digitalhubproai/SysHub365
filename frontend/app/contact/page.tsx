...
export default function Contact() {
  ...
  return (
    <main className="relative bg-[var(--obsidian-base)] selection:bg-neon-accent selection:text-black overflow-x-hidden pt-24 sm:pt-28 md:pt-32 pb-16 sm:pb-20">
      ...
      {/* Hero Section */}
      <section className="relative px-4 sm:px-6 md:px-12 lg:px-24 py-10 sm:py-12 md:py-16 overflow-hidden flex flex-col justify-center">
        <div className="max-w-[90rem] mx-auto w-full relative z-10 grid lg:grid-cols-12 gap-8 sm:gap-12 lg:gap-16 items-start">
          
          {/* Left: Info */}
          <div className="lg:col-span-5 flex flex-col gap-8 sm:gap-10 lg:gap-12 pt-0 sm:pt-4 md:pt-8">
            <div className="flex flex-col gap-4 sm:gap-5 md:gap-6">
              <motion.div initial="hidden" animate="show" variants={fadeUp} className="inline-flex items-center gap-3 px-4 sm:px-5 py-2 rounded-full border border-white/10 bg-white/[0.03] backdrop-blur-md self-start">
                 <span className="text-[10px] sm:text-xs font-bold uppercase tracking-widest text-white/80">Get in Touch</span>
              </motion.div>
              <motion.h1 
            ...
            >
               <div className="group p-4 sm:p-5 md:p-6 rounded-2xl border border-white/5 bg-white/[0.02] hover:bg-white/[0.04] transition-all duration-300 flex items-center gap-4 sm:gap-5 md:gap-6">
                  <div className="w-10 sm:w-12 h-10 sm:h-12 rounded-xl bg-electric-blue/10 flex items-center justify-center text-electric-blue transition-transform group-hover:scale-105 shrink-0">
                      <LuMail size={20} />
                   </div>
                   <div className="flex flex-col min-w-0">
                      <span className="text-[10px] sm:text-xs font-bold text-slate-500 uppercase tracking-widest">Email Us</span>
                      <a href="mailto:hello@syshub365.com" className="text-base sm:text-lg font-bold text-white hover:text-electric-blue transition-colors truncate">hello@syshub365.com</a>
                   </div>
                </div>
          ...
          >
            <PremiumCard className="p-5 sm:p-6 md:p-8 lg:p-12 shadow-2xl">
              <div className="absolute top-0 right-0 p-4 sm:p-6 md:p-8 opacity-10">
                  <LuBuilding2 size={60} className="text-white" />
              </div>
              ...
              <div className="flex flex-col gap-6 sm:gap-8 md:gap-10 relative z-10">
                  <div className="flex flex-col gap-1 sm:gap-2 text-center md:text-left">
                    <h2 className="text-2xl sm:text-3xl font-extrabold text-white tracking-tight">Send a Message</h2>
                  ...
                  <form className="flex flex-col gap-5 sm:gap-6 md:gap-8" onSubmit={handleSubmit}>
                    ...
                    <div className="grid md:grid-cols-2 gap-4 sm:gap-5 md:gap-6">
                      <div className="flex flex-col gap-1 sm:gap-2">
                        <label className="text-[10px] sm:text-xs font-bold text-white/60 uppercase tracking-widest px-2">Full Name</label>
                        <div className="relative">
                          <LuUser className="absolute left-3 sm:left-4 top-1/2 -translate-y-1/2 text-white/30" size={16} />
                          <input
                            type="text"
                            required
                            value={formData.name}
                            ...
                            className="w-full bg-white/[0.03] border border-white/10 rounded-xl py-3 sm:py-4 pl-10 sm:pl-12 pr-3 sm:pr-4 text-sm sm:text-base text-white focus:outline-none focus:border-electric-blue transition-colors placeholder:text-white/20"
                          />
                        </div>
                      </div>
                      <div className="flex flex-col gap-1 sm:gap-2">
                        <label className="text-[10px] sm:text-xs font-bold text-white/60 uppercase tracking-widest px-2">Phone Number</label>
                        <div className="relative">
                    ...
                    {/* Email Address */}
                    <div className="flex flex-col gap-1 sm:gap-2">
                      <label className="text-[10px] sm:text-xs font-bold text-white/60 uppercase tracking-widest px-2">Email Address</label>
                      <div className="relative">
                        <LuMail className="absolute left-3 sm:left-4 top-1/2 -translate-y-1/2 text-white/30" size={16} />
                        <input
                          type="email"
                          required
                    ...
                    {/* Project Details */}
                    <div className="flex flex-col gap-1 sm:gap-2">
                      <label className="text-[10px] sm:text-xs font-bold text-white/60 uppercase tracking-widest px-2">Project Details</label>
                      <div className="relative">
                        <LuMessageSquare className="absolute left-3 sm:left-4 top-4 sm:top-5 text-white/30" size={16} />
                        <textarea
                          rows={4}
                          required
                          ...
                          className="w-full bg-white/[0.03] border border-white/10 rounded-xl py-3 sm:py-4 pl-10 sm:pl-12 pr-3 sm:pr-4 text-sm sm:text-base text-white focus:outline-none focus:border-electric-blue transition-colors placeholder:text-white/20 resize-none"
                        />
                      </div>
                    </div>
                    {status.type && (
                      <div className={`flex items-center gap-3 p-3 sm:p-4 rounded-xl border ${
                        ...
                        {status.type === "success" ? <LuCheck size={16} /> : <LuTriangleAlert size={16} />}
                        <span className="text-xs sm:text-sm font-medium">{status.text}</span>
                      </div>
                    )}
                    ...
                    <Button 
                      ...
                      className="w-full !rounded-xl gap-3 mt-2 sm:mt-4"
                    >
                      <span className="text-sm sm:text-base">{isSubmitting ? "Sending..." : "Send Message"}</span>
                      <LuSend size={16} className="hidden sm:inline-block" />
                    </Button>
                  </form>
              </div>
            </PremiumCard>
      ...
      <section className="py-16 sm:py-20 md:py-24 px-4 sm:px-6 md:px-12 lg:px-24">
         <div className="max-w-[90rem] mx-auto flex flex-col gap-10 sm:gap-12 md:gap-16">
            <div className="flex flex-col gap-3 sm:gap-4 text-center md:text-left">
               <span className="text-electric-blue font-black tracking-[0.3em] uppercase text-xs sm:text-sm">Our Offices</span>
               <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-extrabold text-white tracking-tight leading-tight">Global Presence.</h2>
            </div>
            
            <div className="grid gap-6 sm:gap-8 max-w-2xl mx-auto md:mx-0 w-full">
                {[
